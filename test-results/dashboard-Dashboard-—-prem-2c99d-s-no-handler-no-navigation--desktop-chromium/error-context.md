...
- Name: dashboard.spec.ts >> Dashboard — premium user >> BUG: AI 'Deeper analysis' button has no handler (no navigation)
...
Call log:
  - waiting for getByRole('button', { name: /^Deeper analysis$/ })
    - locator resolved to <button type="button" class="mt-4 inline-flex items-center gap-1.5 text-[12.5px] text-[var(--accent)] hover:underline">…</button>
