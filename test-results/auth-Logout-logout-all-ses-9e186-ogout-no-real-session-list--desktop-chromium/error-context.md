...
- Name: auth.spec.ts >> Logout >> logout-all-sessions in Settings does the same as logout (no real session list)
- Location: tests/e2e/auth.spec.ts:141:7
...
Call log:
  - waiting for getByRole('button', { name: /Log out of all devices/ })
    - locator resolved to <button type="button" class="btn-ghost !text-[12.5px] !py-1.5 shrink-0">Log out of all devices</button>
