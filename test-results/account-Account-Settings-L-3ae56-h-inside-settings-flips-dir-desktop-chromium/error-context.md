...
- Name: account.spec.ts >> Account / Settings >> Language switch inside settings flips dir
- Location: tests/e2e/account.spec.ts:82:7
...
Call log:
  - waiting for getByRole('button', { name: /^العربية$/ })
    - locator resolved to <button type="button" aria-pressed="false" class="px-3 py-1.5 rounded-full transition-colors">العربية</button>
