...
- Name: account.spec.ts >> Account / Settings >> Delete account confirm → wipes user + watchlist, returns to /
- Location: tests/e2e/account.spec.ts:69:7
...
Call log:
  - waiting for getByRole('button', { name: /^Delete account$/ })
    - locator resolved to <button type="button" class="inline-flex items-center justify-center gap-1.5 rounded-full px-4 h-9 text-[12.5px] font-medium transition-colors text-white">…</button>
